...
##' @return A list containing the information of the survival forest fit.
##' @examples 
##' \donttest{
##' library(survival)
##' a_survival_forest<-
...
survival_forest<-function(
  ...
  # if(!all(sapply(mf_numeric_predictor,class)%in%c("integer","numeric")))stop("Invalid 'numeric_predictor' formula")
  # if(!all(sapply(mf_factor_predictor,class)%in%c("factor","character")))stop("Invalid 'factor_predictor' formula")
  ...
  for(boot_idx in 1:nboot){
    ...
    matrix_numeric_boot<-matrix_numeric[shuffle,,drop=F]
    matrix_factor_boot<-matrix_factor[shuffle,,drop=F]
